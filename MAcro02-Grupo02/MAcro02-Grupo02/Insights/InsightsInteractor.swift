...
class InsightsInteractor : InsightsInteractorProtocol {
    ...
    init(){
        Task{
            await testCloudDatas()
        }
    }
    func testCloudDatas() async{
        let dataManager = PomodoroData()
        do{
            let data1 = try await dataManager.savePomodoro(focusTime: 25, breakTime: 5, date: Date(), tag: "study")
            ...
            // Converte DateComponents para Date usando o calendário atual
            if let specificDate = Calendar.current.date(from: dateComponents) {
                let data2 = try await dataManager.savePomodoro(focusTime: 45, breakTime: 10, date: specificDate, tag: "study")
                let data3 = try await dataManager.savePomodoro(focusTime: 50, breakTime: 15, date: specificDate, tag: "work")
            } else {
                print("Data inválida.")
            }
