...
class PomodoroViewController: UIViewController, UIPopoverPresentationControllerDelegate {
    ...
    @objc private func didTapPlayPause() {
        ...
        // Adjust these durations and settings as needed for testing
        interactor?.startPomodoro(
            workDuration: 1,
            breakDuration: 1,
            loopCount: 2,
            longRestDuration: 1,
            wantsBreathing: true // Change to false if breathing is not desired
        )
    }
    ...
    func updateButton(isRunning: Bool, isPaused: Bool) {
        ...
        resumeButton.isHidden = !isPaused || isRunning
        resetButton.isHidden = !isPaused
    }
    ...
    func updateTimerCircleProgress(_ progress: Float) {
        progressCircleView.progress = progress
        progressCircleView.setNeedsDisplay() // Ensure the view is redrawn
        }
